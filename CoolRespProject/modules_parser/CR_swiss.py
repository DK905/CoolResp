r"""Вторичные функции для взаимодействия с расписанием.

    - check_file_extension() : выделение расширения файла
    - json_to_database()     : загрузка БД из JSON
    - database_to_json()     : сохранение БД в JSON
    - take_data()            : выборка респы для подгрупп
    - take_time()            : определение периода расписания
    - use_standard()         : стандартизация датафрейма
    - create_name()          : формирование оптимального названия для файла расписания
    - format_item_name()     : форматирование предмета
    - format_teacher()       : форматирование препода
    - format_pdgr()          : форматирование подгруппы
    - format_cab()           : форматирование кабинета
    - df_differences()       : для разницы между датафреймами
    - replace_type()         : замена типа данных по словарю
    - find_bad_type()        : поиск косячных типов пар
...
def string_float_to_string_int(some_string: 'Строка, которая может являться float-числом'
    ...
    # xlrd считывает числовые строки (такие как «881») в float формате (как «881.0»)
    # Проверка isdigit() не отловит «881.0» как число, однако объекты вроде названий групп должны быть «881»
    # Поэтому, вместо if group.isdigit() используется try-except, гарантированно регламентирующий название группы
    try:
        group = str(int(some_string))
    except:
        group = some_string
    finally:
        return group
...
def json_to_database(path_load: 'Путь к базе данных'
                     ) -> 'Датафрейм БД':
    """ Функция загрузки базы данных расписания из JSON """

    # Считывание файла БД из JSON
    df = pd.read_json(path_load)

    # Применение технической стандартизации к датафрейму
    df = use_standard(df)

    # Попытка взять название группы из названия файла
    df.name = create_name(df)

    return df


def database_to_json(df: 'Датафрейм с БД расписания, сохраняемый в JSON',
                     ps: 'Путь к каталогу сохранения файла'
                     ) -> None:
    ...
    # Сохранение в JSON
    df.to_json(os.path.join(ps, f'{name}.json'),  # Имя файла
...
def take_data(df:  'БД с запарсенной инфой',
    ...
    my_grps = {a[0]: (a[1] if not a[1] in [0, '0'] else crd.DEF_GROUPS) for a in {0: 0,   # 0 подгрупп: общая пара
