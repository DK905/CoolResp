...
from re import findall, search, split as rsplit, sub

# Импорт команд для обработки дат из модуля datetime
from datetime import date as dt_date, timedelta

# Импорт умолчаний и сокращений для типов пар
from CoolRespProject.modules.CR_dataset import defaults, tip_list
...
def format_prep(lecturer: 'Подстрока форматируемого препода',
    ...
    if not search(r'(?m)\.$', lecturer):
        lecturer += '.'

    # Поставить пробел между фамилией и инициалами, доставить должность если нужно
    prep = ['', search(prep_pat2, lecturer)[1], search(prep_pat2, lecturer)[2]]
    if dv_yn or not search(prep_pat1, lecturer):
        del(prep[0])
    else:
        prep[0] = search(prep_pat1, lecturer)[1]
...
def parser(trash: 'База обработки',
    ...
    # Даты
    pat3 = r'(?m)(?:(?:с\s*(?:\d{2}[.,г;\s]*\s*)+по\s*(?:\d{2}(?:[.,г;\s]|$)\s*)+))|(?:(?:\d{2}(?:[.,г;\s]|$)\s*)+)'
    # Отлов подгрупп
    pat4 = r'\d\s*[п]?\s*/\s*гр'
    # Отлов типа пары
    ...
    for record in trash:
        ...
        while True:
            ...
            if len(findall(pat1, rec)) > 1:
                lecturer = search(pat2, rec)[0] # Для комфортного добавления препода в строку
            ...
            if len(findall(pattern2a, divide[ind])) > 1:
                ...
                if not search(pat3, sub(pattern5, '', divide[ind])):
                    ...
                    for pp in findall(pattern2b, part2):
                        ...
                        parse_list.append('; '.join([sub(pat5, lambda m: m[0] + ': ' + pp[0], sub(pat4, '', part1)), pp[1]]))
                else:
                    # Шаг 1) Вырезать предмет (перед ним может быть дата, если первый тип - лекция/практика)
                    edu_pair = search(pat1, divide[ind])[1]
                    for_sep = sub(edu_pair, '', divide[ind])
            ...
            for logic_set in parse_list:
                ...
                if search(pat1, logic_set):
                    edu_pair = search(pat1, logic_set)[1]
                    # Если предметы сокращаются, то предмет - сокращённая запись
                    if a_yn:
                        edu_pair = format_edu_pair(edu_pair)
                    logic_set = sub(pat1, '', logic_set)
                else: # Если предмета нет (такого быть не должно, но мало ли)
                    edu_pair = 'Not found'

                """ Выделение наборов "даты - тип пары - группа" """
                it_inf = iter([a for a in findall(pattern4, logic_set) if a])  # Типологический итератор
                ...
                for f in range(len(dtg)):
                    ...
                    if not dtg[f][1]:  # Иногда, тип пары всё равно отсутствует
                ...
                if findall(pat2, logic_set):  # Если есть препод, отловить его с должностью (если указана)
                    lecturer = [format_prep(findall(pat2, logic_set)[0], p_yn)]
                else:  # Если препод не указан
                    if findall(pat5, parse_list[i]):  # Если у текущей пары есть тип, то она не "особая"
                        lecturer = [format_prep(parse[-1][3], p_yn)]
                ...
                for info in dtg:
                    ...
                    if len(parse) and info[1][0]==defaults[1]:
                        if parse[-1][2] == edu_pair:  # Если предмет как в предыдущем, а тип базовый, то ошибка в типе
                            info[1] = [parse[-1][4]]
...
def print_bd(bd:     'База парсинга',
    ...
    """ Функция для консольного вывода базы парсинга (только для ручного тестирования) """
