""" Информация о модуле
Данный модуль предназначен для хранений функций работы с базой данных расписания
...
- create_name() : сформировать оптимальное название расписания
...
def create_name(df:  'Датафрейм базы данных расписания',
    ...
    time = ' - '.join(d.strftime('%d.%m.%Y') if not d is np.nan else '' for d in take_time(df))
...
- : Форматирование препода
- : форматирование подгруппы
- : форматирование кабинета
...
def database_to_json(df: 'Датафрейм с БД расписания, сохраняемый в JSON',
    ...
    # Сохранение в JSON
    df.to_json(f'{ps}{name}.json',  # Имя файла
