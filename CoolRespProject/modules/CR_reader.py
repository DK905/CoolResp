...
def group_choice(sheet: 'Объект листа с загруженной книги'
    ...
    if period and grp_list and ind_start and ind_end:
        return {'period': period, 'year': year, 'groups_info': grp_list, 'range': (ind_start + 1, ind_end)}
    else:
        return {'period': '', 'year': 0, 'groups_info': [], 'range': (0, 0)}
...
def prepare(sheet: 'Выбранный лист',
    ...
    # xlrd считывает название числовых групп (таких как 881) в float формате (как 881.0)
    # Проверка isdigit() не отловит '881.0' как число, однако название группы должно быть '881'
    # Поэтому, вместо if group.isdigit() используется try-except, гарантированно регламентирующий название группы
    try:
        group = str(int(group))  # Для групп вроде '881'
    except:
        pass  # Для обычных групп
