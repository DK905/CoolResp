...
1) **Простой запуск для Windows:**

- Загрузить исполняемый файл `CoolResp.exe`  из корневого каталога в репозитории;
- Запустить скачанную программу;
- Начать работу с конвертером. Подробнее об его интерфейсе можно прочесть [тут](#Навигация-по-интерфейсу).   

2) **Проектный запуск на другой системе:**

- Загрузить из репозитория папку исходных кодов проекта `CoolRespProject` и  инициализатор `main.py`;
- Установить `python 3.8.3` и все необходимые зависимости...  
	+ ...вручную, поставив основные зависимости из [списка](#);
	+ ...автоматически, подгрузив их командой `pip install -r requirements.txt` из `requirements.txt`;
-  Запустить проект через `main.py` и начать работу с конвертером. [Интерфейс конвертера](#Навигация-по-интерфейсу).  
...
<p>
	<img src="Design/cr_interface_no_sign.png" alt="Интерфейс CoolResp" width="400" height="480">
	<img src="Design/cr_interface_with_sign.png" alt="Пометки" width="400" height="480">
</p>
