...
def use_standard(df:  'Датафрейм базы данных расписания'
    ...
    df['cab'] = df['cab'].fillna(cr_def.DEF_CABS)
    ...
    df['day'] = df['day'].astype(pd.CategoricalDtype(cr_def.DAYS_NAMES,  # Список категорий = дни
...
def take_data(df:  'БД с запарсенной инфой',
    ...
    my_grps = {a[0]: (a[1] if not a[1] in [0, '0'] else
                      cr_def.DEF_GROUPS) for a in {0: 0,  # 0 подгрупп: общая пара
    ...
    count_groups = lambda vals: len([g for g in set(vals) if g != cr_def.DEF_GROUPS])
    ...
    df_merge = df_merge.query('(pdgr == @cr_def.DEF_GROUPS) | (grps == @cr_def.DEF_GROUPS) | (pdgr == grps)').drop('grps', axis=1).reset_index(drop=True)
...
def find_bad_type(parse: 'БД с запарсенной инфой'
    ...
    a_bd = parse.query('pdgr != @cr_def.DEF_GROUPS').groupby(['item_name',
