...
def get_cell_value(row: 'Координата строки',
    ...
    if not act:
        ...
        if indicator:
            ...
            indicator = get_cell_value(row, diapason[3], mgl, True)
        # Если актуальная ячейка - обычная
        else:
            # Получить значение из правого столбца
            indicator = get_cell_value(row, col + 1, mgl, True)
...
def prepare(sheet: 'Выбранный лист',
    ...
    for row in range(*coord):
        ...
        act_day = sheet.cell(*get_cell_value(row, 0, merge_coords, True)).value

        # Номер пары берётся из первой ячейки строки
        act_num = sheet.cell(*get_cell_value(row, 1, merge_coords, True)).value

        # Информация о парах берётся из главной ячейки группы
        act_rec = sheet.cell(*get_cell_value(row, g, merge_coords, True)).value

        # Информация о кабинетах берётся следующей ячейкой, от правой ячейки относительно главной
        act_cab = sheet.cell(*get_cell_value(row, g, merge_coords, False)).value
