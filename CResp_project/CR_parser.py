...
def expand_dates(dates, year, day):
    ...
    for date in dates:
        ...
        if search(dat_pat, date):
            ...
            dt_start = list(map(int, split(r'\.', start_end[0])))
            dt_final = list(map(int, split(r'\.', start_end[1])))
...
Известные баги:
    1) Неправильно парсится список кабинетов, т.к не проработана логика распределения кабинетов
